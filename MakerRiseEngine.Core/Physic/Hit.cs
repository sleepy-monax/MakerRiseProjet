...
namespace Maker.RiseEngine.Core.Physic
{
    public class Hit : IHit
    {
        ...
        private static Tuple<Vector2, Vector2> PushOutside(Vector2 origin, RectangleF other)
        {
            ...
            if (Math.Abs(min - top) < Values.Threshold)
            {
                normal = -Vector2.UnitY;
                position = new Vector2(position.X, other.Top);
            }
            else if (Math.Abs(min - bottom) < Values.Threshold)
            {
                normal = Vector2.UnitY;
                position = new Vector2(position.X, other.Bottom);
            }
            else if (Math.Abs(min - left) < Values.Threshold)
            {
                normal = -Vector2.UnitX;
                position = new Vector2(other.Left, position.Y);
            }
            else if (Math.Abs(min - right) < Values.Threshold)
            {
                normal = Vector2.UnitX;
        ...
        private static Hit ResolveNarrow(RectangleF origin, RectangleF destination, RectangleF other)
        {
            ...
            if (Math.Abs(velocity.X) < Values.Threshold)
            {
            ...
            if (Math.Abs(velocity.Y) < Values.Threshold)
            {
        ...
        private static Vector2 GetNormal(Vector2 invEntry, Vector2 invExit, Vector2 entry)
        {
            if (entry.X > entry.Y)
            {
                return (invEntry.X < 0.0f) || (Math.Abs(invEntry.X) < Values.Threshold && invExit.X < 0) ? Vector2.UnitX : -Vector2.UnitX;
            }

            return (invEntry.Y < 0.0f || (Math.Abs(invEntry.Y) < Values.Threshold && invExit.Y < 0)) ? Vector2.UnitY : -Vector2.UnitY;
        }
