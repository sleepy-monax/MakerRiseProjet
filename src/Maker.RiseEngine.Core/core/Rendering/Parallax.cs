...
namespace Maker.RiseEngine.Core.Rendering
{
    public class Parallax : IDrawable
    {
        ...
        public Parallax(ParallaxLayer[] layers, Rectangle _DistinationRectangle)
        {
    ...
    public static class ParallaxParse
    {
        ...
        public static Parallax Parse(string _PluginName, string _Name, Rectangle _DistinationRectangle)
        {
            ...
            for (int i = 0; i < Ls.Length; i++)
            {
                ...
                if (sub.Length == 2)
                    paralaxeLayerList.Add(new ParallaxLayer(Rise.Engine.ressourceManager.GetTexture2D(_PluginName, "parallax\\" + _Name + "\\" + sub[0]), float.Parse(sub[1], culture)));
