...
namespace Maker.RiseEngine.Core.Network
{
    public class HTTPServer
    {
        ...
        public bool Start(IPAddress serverIpAddress, int port, int maxClientCount, string contentPath)
        {
            bool result;
            ...
            else
            {
                ...
                try
                {
                    ...
                    this.serverSocket.Bind(new IPEndPoint(serverIpAddress, port));
                ...
                ThreadStart MainThreatLoop = new ThreadStart(delegate
               {
                ...
                Thread serverThread = new Thread(MainThreatLoop);
        ...
        private void HandleClientRequest(Socket clientSocket)
        {
            ...
            int byteCount = clientSocket.Receive(array);
            string clientrequestString = this.charEncoder.GetString(array, 0, byteCount);
            string text = clientrequestString.Substring(0, clientrequestString.IndexOf(" "));
            int num = clientrequestString.IndexOf(text) + text.Length + 1;
            int num2 = clientrequestString.LastIndexOf("HTTP") - num - 1;
            string text2 = clientrequestString.Substring(num, num2);
