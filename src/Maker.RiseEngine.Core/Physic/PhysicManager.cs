...
using Maker.RiseEngine.Core.Physic.Responses;
...
namespace Maker.RiseEngine.Core.Physic
{
    ...
    {
        public PhysicManager(float width, float height, float cellSize = 64)
        {
        ...
        public IHit Hit(Vector2 point, IEnumerable<IBox> ignoring = null)
        {
            ...
            {
                var hit = Physic.Hit.Resolve(point, other);
        ...
        public IHit Hit(Vector2 origin, Vector2 destination, IEnumerable<IBox> ignoring = null)
        {
            ...
            foreach (var other in boxes)
            {
                var hit = Physic.Hit.Resolve(origin, destination, other);
